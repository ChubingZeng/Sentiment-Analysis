...
test_features.shape
train_y = np.array(list(map(convert_01, train_y)))
test_y = np.array(list(map(convert_01, test_y)))
...
# Predict
pred = dtree_model.predict(test_features)
print(pred)
...
n_classes = 2
y_pred = dtree_model.predict_proba(train_features)
y_pred[:,0]
y_pred_list = np.array([y for x in np.array(y_pred[:,0]) for y in x])
...
def predict_new(sentence: str):
    ...
    pred = dtree_model.predict(features)
    return pred[0]
